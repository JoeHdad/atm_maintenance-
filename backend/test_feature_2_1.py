...
try:
    ...
    if response.status_code == 201:
        print(f"✅ Technician created successfully")
        ...
        if 'password' not in data:
            print(f"✅ Password excluded from response")
        else:
            print(f"❌ WARNING: Password exposed in response!")
    else:
        print(f"❌ Failed to create technician")
except Exception as e:
    print(f"❌ Error: {e}")
...
try:
    ...
    if response.status_code == 400:
        data = response.json()
        print(f"✅ Duplicate username rejected (expected)")
        print(f"   Error: {data}")
    else:
        print(f"❌ Should have rejected duplicate username")
except Exception as e:
    print(f"❌ Error: {e}")
...
try:
    ...
    if response.status_code == 400:
        data = response.json()
        print(f"✅ Weak password rejected (expected)")
        print(f"   Error: {data}")
    else:
        print(f"❌ Should have rejected weak password")
except Exception as e:
    print(f"❌ Error: {e}")
...
try:
    ...
    if response.status_code == 400:
        data = response.json()
        print(f"✅ Non-alphanumeric username rejected (expected)")
        print(f"   Error: {data}")
    else:
        print(f"❌ Should have rejected non-alphanumeric username")
except Exception as e:
    print(f"❌ Error: {e}")

# Test 6: Try without city
print("\n[TEST 6] Try to create technician without city...")
try:
    no_city_data = {
        'username': 'tech_test3',
        'password': 'SecurePass123!'
    ...
    if response.status_code == 400:
        data = response.json()
        print(f"✅ Missing city rejected (expected)")
        print(f"   Error: {data}")
    else:
        print(f"❌ Should have rejected missing city")
except Exception as e:
    print(f"❌ Error: {e}")

# Test 7: Try without authentication
print("\n[TEST 7] Try to create technician without authentication...")
try:
    response = client.post(
        '/api/host/technicians/',
        data=json.dumps({
    ...
    if response.status_code == 401:
        print(f"✅ Unauthenticated request rejected (expected)")
    else:
        print(f"❌ Should have rejected unauthenticated request")
except Exception as e:
    print(f"❌ Error: {e}")

# Test 8: Try with technician role (not host)
print("\n[TEST 8] Try to create technician as technician (not host)...")
try:
    # Login as technician
    ...
    if tech_response.status_code == 200:
        ...
        if response.status_code == 403:
            print(f"✅ Non-host user rejected (expected)")
        else:
            print(f"❌ Should have rejected non-host user")
    else:
        print(f"❌ Could not login as technician")
except Exception as e:
    print(f"❌ Error: {e}")

# Test 9: Verify created technician in database
print("\n[TEST 9] Verify created technician in database...")
try:
    technician = User.objects.filter(username='tech_test1').first()
    
    if technician:
        print(f"✅ Technician found in database")
        ...
        if technician.role == 'technician':
            print(f"✅ Role correctly set to 'technician'")
        else:
            print(f"❌ Role not set correctly!")
    else:
        print(f"❌ Technician not found in database")
except Exception as e:
    print(f"❌ Error: {e}")

# Test 10: Test login with created technician
print("\n[TEST 10] Test login with created technician...")
try:
    response = client.post(
        '/api/auth/login/',
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"✅ Created technician can login")
        print(f"   User: {data.get('user', {}).get('username')}")
        print(f"   Role: {data.get('user', {}).get('role')}")
    else:
        print(f"❌ Created technician cannot login")
        print(f"   Status: {response.status_code}")
except Exception as e:
    print(f"❌ Error: {e}")
